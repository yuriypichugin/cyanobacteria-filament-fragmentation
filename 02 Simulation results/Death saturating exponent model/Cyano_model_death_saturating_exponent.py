...
import Cyano_model_support_v3_3 as CMS
...
#JobID = 0
FileName = "Results/CyanoFrag_Fitting_Sigmoid_Death_model_"+str(JobID)+".txt"
File2Write = open(FileName, "w")
String2Write = "W, K, Scale, Min_effect, Max_effect, empty_2, empty_3, empty_4, P_comp, D_comp, Deviation"+'\n'
File2Write.write(String2Write)

""" initialization """

TheModel = CMS.SaturatedExponentDeathModel

W = np.power(10, np.random.uniform(-3.0, 0.0))
K = np.power(10, np.random.uniform(3, 4))
P_comp = 1.0
D_comp = np.power(10, np.random.uniform(-2.0, 0.5))
Scale = np.power(10, np.random.uniform(0, 4))
#Transition_width = np.power(10, np.random.uniform(0, 4))
Min_effect = np.power(10, np.random.uniform(-4, 0))
Max_effect = np.power(10, np.random.uniform(-4, 0))
Parameters = [W, K, Scale, Min_effect, Max_effect, 0, 0, 0, P_comp, D_comp]


#Conc_treshold = np.power(10, np.random.uniform(0, 4))
#Max_effect = np.power(10, np.random.uniform(-4, 0))
#Parameters = [W, K, Conc_treshold, Max_effect, 0, 0, 0, 0, P_comp, D_comp]
