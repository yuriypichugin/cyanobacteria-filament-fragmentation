...
#JobID = 0
FileName = "Results/CyanoFrag_Fitting_decay_exponent_consumption_model_"+str(JobID)+".txt"
File2Write = open(FileName, "w")
String2Write = "W, K, Scale, Min_effect, Max_effect, empty_2, empty_3, Flag_consumption_model, Init_conc_comp, D_comp, Deviation"+'\n'
File2Write.write(String2Write)

""" initialization """

TheModel = CMS.DecayingExponentConsumptionModel
...
Scale = np.power(10, np.random.uniform(-2, 2))
a_val = np.power(10, np.random.uniform(-2, 2))
b_val = np.power(10, np.random.uniform(-2, 2))
Min_effect = np.min([a_val,b_val])
Max_effect = np.max([a_val,b_val])

Parameters = [W, K, Scale, Min_effect, Max_effect, 0, 0, 1, Init_conc_comp, D_comp]
