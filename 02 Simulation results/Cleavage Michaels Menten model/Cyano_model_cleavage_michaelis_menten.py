...
#JobID = 0
FileName = "Results/CyanoFrag_Fitting_Michaelis_Menten_Cleavage_model_"+str(JobID)+".txt"
File2Write = open(FileName, "w")
String2Write = "W, K, Kin_const, Max_effect, empty_1, empty_2, empty_3, empty_4, P_comp, D_comp, Deviation"+'\n'
File2Write.write(String2Write)

""" initialization """

TheModel = CMS.MichaelisMentenCleavageModel

W = np.power(10, np.random.uniform(-3.0, 0.0))
K = np.power(10, np.random.uniform(3, 4))
P_comp = 1.0
D_comp = np.power(10, np.random.uniform(-2.0, 0.5))
Kin_const = np.power(10, np.random.uniform(0, 4))
Max_effect = np.power(10, np.random.uniform(-4, 0))
Parameters = [W, K, Kin_const, Max_effect, 0, 0, 0, 0, P_comp, D_comp]
