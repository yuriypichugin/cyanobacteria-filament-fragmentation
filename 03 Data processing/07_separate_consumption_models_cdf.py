...
#	MaskAll = np.asarray( 6*[1] )
	...
	DataFromModels = len(FileNamesList_consumption)*[0]
	for i in np.arange(len(DataFromModels)):
		DataFromModels[i] = pd.read_csv(Folder_name_consumption + FileNamesList_consumption[i], sep = ',') 
...
#
""" Deviation histograms for _all_ cleavage models """
MinErr = min(DevCNS)
MaxErr = 0.35
BinsList = np.linspace(MinErr, MaxErr, num = 30)
FileName = '07_consumption_models_all_cdf.png'
""" make figure """
fig, ax = plt.subplots(1,1)
HTYPE = 'step'
CLR = ['#e41a1c', '#377eb8', '#4daf4a', '#984ea3', '#ff7f00', '#f781bf', '#a65628', '#999999']
...
#		plt.hist(Dev, BinsList, histtype = HTYPE, color = CLR[counter], linewidth = 3)
		counter += 1
...
counter = 0
LineHeight = [1.0, 1.0, 0.9, 1.0, 0.8, 1.0, 0.7, 1.0]
for i in np.arange(len(DataFromModels)):
	if MaskConsumption[i] == 1:
		Dev = DataFromModels[i]['Deviation']/NoGrowthDeviation
...
""" Deviation histograms for _selected_ cleavage models """
MaskSelected = [1, 0, 0, 1, 0, 0, 1, 1]


MinErr = min(DevCNS)
MaxErr = 0.35
BinsList = np.linspace(MinErr, MaxErr, num = 30)
FileName = '07_consumption_models_all_cdf_v1_1.png'
""" make figure """
fig, ax = plt.subplots(1,1)
HTYPE = 'step'
CLR = ['#e41a1c', '#377eb8', '#4daf4a', '#984ea3', '#ff7f00', '#f781bf', '#a65628', '#999999']
...
counter = 0
LineHeight = [1.0, 1.0, 0.9, 0.7, 0.8, 1.0, 0.5, 0.2]
for i in np.arange(len(DataFromModels)):
	if MaskConsumption[i] == 1:
		Dev = DataFromModels[i]['Deviation']/NoGrowthDeviation
		if MaskSelected[i]==1:
